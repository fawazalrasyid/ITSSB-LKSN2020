...
function register_user($p_name, $p_email, $p_password, $p_registeredNumber, $p_imagePath){
  ...
  $query = "INSERT INTO teacher(name, email, password, registered_number, image_path) VALUES('$nama', '$email', '$encrypted_pass', '$registNumber', '$imagePath')";
  
  $user_new = mysqli_query($conn, $query);
  if( $user_new ) {
    $usr = "SELECT * FROM teacher WHERE email = '$email'";
...
function cek_username($p_email){
  global $conn;
  $query = "SELECT * FROM teacher WHERE email = '$p_email'";
